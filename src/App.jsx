...
export default function App() {
  ...
  const sendMessage = async () => {
    ...
    setAiLoading(true);
    try {
      // Calls our own Vercel serverless function — no CORS issues, API key stays hidden
      ...
      if (data.error) {
        ...
        let errMsg = `❌ Error: ${data.error?.message || data.error}`;
        if (t === 'authentication_error') errMsg = "❌ Invalid API key.\n\nFix: Vercel → Settings → Environment Variables → check ANTHROPIC_API_KEY starts with sk-ant- and has no spaces. Then Redeploy.";
        else if (t === 'rate_limit_error') errMsg = "⏳ Rate limit hit. Wait 30 seconds and try again.";
        else if (t === 'insufficient_quota') errMsg = "💳 No credits left. Add credits at console.anthropic.com → Billing.";
        setMessages(prev => [...prev, { role: 'assistant', content: errMsg }]);
      } else {
        const reply = data.content?.[0]?.text || 'No response received. Try again.';
